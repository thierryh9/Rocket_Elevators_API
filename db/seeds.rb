...
100.times do |n|
 ...
 rand(2).times do |x|
	...
	rand(4).times do
		#elevator
		elevator = Elevator.create(serialNumber: Faker::Device.serial,installDate: Faker::Date.between(from: '2019-09-23', to: '2020-01-25'),inspectionDate: Faker::Date.between(from: '2019-09-23', to: '2020-01-25'),status: rand(1),certificat: Faker::Device.serial,information: Faker::Types.rb_string , note: Faker::Types.rb_string , column: col,type: Type.offset(rand(Type.count)).first, category: Category.offset(rand(Category.count)).first)
	end
 end
end
2.times do |i|
